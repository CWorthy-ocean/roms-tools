...
* Extracted shared lateral-regrid helpers (`build_lateral_regridder`, `select_source_mask` in `regrid.py`; `check_source_coverage`, `apply_source_prefill`, `apply_scipy_fallback_fill` in `setup/utils.py`); `BoundaryForcing` and `InitialConditions` now share this machinery (no user-facing change for `BoundaryForcing`). ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
