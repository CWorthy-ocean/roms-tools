...
* `BoundaryForcing` is now a wrapper over multiple constituent sources, and the previous single-source class is available as `BoundaryForcingSource`. The wrapper takes the physics dataset as `source` plus a list of `bgc_sources`, and exposes the constituent objects as `.physics` and `.bgc` (a list); it does **not** accept `type=` or `physics_forcing=`, and has no `.ds` or `.plot()` of its own. Code that built a single boundary-forcing object continues to work unchanged by renaming the class to `BoundaryForcingSource`.
* `BoundaryForcing.save()` now takes `save(physics_filepath, bgc_filepaths=None, group=True, format=..., serialize_dask=None)` and returns `(physics_paths, bgc_paths)`, rather than taking a single `filepath`. `group`/`format`/`serialize_dask` are honoured by every BGC companion exactly as they are by the physics object, and `bgc_paths` is a `list[list[Path]]` of the actual files written (one list per source, in `.bgc` order) rather than merely echoing back the requested filenames — so a `group=True` split into multiple files is reflected directly. Each BGC source is written to its own file — ROMS's `frcfiles` namelist key accepts a list — so, unlike `InitialConditions`, there is no merge into one dataset.
* `bgc_model` is now required whenever `bgc_source` or `bgc_sources` is given, on both `InitialConditions` and `BoundaryForcing`; omitting it raises `ValueError`. Pass `bgc_model=BGCMarbl` to reproduce the previous behaviour.
* `InitialConditions` is a smaller change: it keeps `source`, `bgc_source`, `use_vars`, `.ds` and `.plot()`, and additionally gains `bgc_sources`, `bgc_model`, `.physics` and `.bgc`, so existing code needs only the `bgc_model` addition above. Its single-source class is available as `InitialConditionsSource`.

### New Features

* `InitialConditions` and `BoundaryForcing` now accept **multiple BGC sources** through `bgc_sources`, a list of entries of the form `{"source": {...}, "use_vars": [...]}`. Each entry is regridded independently and may be narrowed with `use_vars`, so a run can draw `NO3`/`PO4`/`SiO3`/`O2` from one dataset and `DIC`/`ALK`/`Fe` from another without the sources overlapping. `bgc_source` is still accepted for a single source.
* Added `"WOA"` as a gridded BGC source for `InitialConditions` and `BoundaryForcing`, reading World Ocean Atlas 2023 nutrients and oxygen (`NO3`, `PO4`, `SiO3`, `O2`) directly from NCEI. The source also loads the matching monthly temperature and salinity, which it uses to convert the tracers from µmol kg⁻¹ to mmol m⁻³ and to supply the source density coordinate for `density`/`density_mld` interpolation. WOA23 publishes nutrients and oxygen on the 1° grid only, so the source is 1° throughout. Monthly WOA fields stop at 800 m (nutrients) and 1500 m (oxygen, T/S), so each variable is extended onto the full-depth 102-level annual grid according to a new `deep_fill` option: `"annual_blend"` (default) splices the annual climatology underneath with a linear taper centred on each variable's own seam (half-width `deep_blend_halfwidth`, default 100 m, giving 700–900 m for the nutrients), while `"ffill"` persists the deepest monthly value downward. Omitting `path` downloads the files to the `roms-tools` cache via the new `download_woa23_bgc()` helper.
* Added `"GLODAP"` and `"constants"` BGC sources for `InitialConditions` and `BoundaryForcing`, which have no time axis. For boundary forcing, ROMS-Tools writes two records bracketing the requested start and end times so the values stay constant over the simulation; for initial conditions the fields take the physics time. `"GLODAP"` supplies spatially and depth-resolved fields for `ALK`, `DIC`, `NO3`, `PO4`, `SiO3` and `O2`.
* Added `BGCModel` and its `BGCMarbl` implementation. The BGC tracer set is now completed by this model class rather than by hard-coded helpers in `setup/utils.py`, with every MARBL tracer derived from a single rule table: which tracers exist, which are copied from another (`DIC` → `DIC_ALT_CO2`, `ALK` → `ALK_ALT_CO2`), which are derived (`Fe` → `Lig`; `CHL` → the small-phytoplankton / diatom / diazotroph / zooplankton set), and which fall back to a constant open-ocean value.
* Added `"ESPER"` as a BGC source for `InitialConditions` and `BoundaryForcing`. Rather than regridding a dataset, it derives BGC fields from the physics temperature and salinity already on the ROMS grid using the PyESPER empirical routines, so it needs no dataset load and no lateral or vertical regridding — it is handled like the `constants` source. **Support is experimental**, and requires [CWorthy's PyESPER fork](https://github.com/CWorthy-ocean/PyESPER) installed from source (`pip install -e .`).
* `roms_tools.utils.save_datasets` gained `serialize_dask` and `show_progress`, and the forcing objects' `save()` methods gained `serialize_dask`. Setting it runs the write one dask task at a time — a manual tool for sources whose per-chunk memory cost makes the ordinary concurrent write untenable (ESPER frequently needs this). It defaults to off, so writes are unchanged unless it is requested.
* `BoundaryForcingSource`, `InitialConditionsSource`, `BGCModel`, `BGCMarbl` and `compute_potential_density` are now exported from `roms_tools` and documented in the API reference.

### Improvements

* Datasets written with `group=True` are now split into per-month (or per-year) files by contiguous index slices rather than by `xarray`'s `groupby`. `groupby` selects with fancy indexing, which re-blocks a dask array and severs the alignment between the blocks being written and the blocks being computed; on a year-long boundary write that cost roughly 2.8x the necessary computation. Selection is contiguous now, so the grouping no longer perturbs chunking. Datasets whose time axis does not group into contiguous runs fall back to the previous `groupby` path.

### Miscellaneous

* New runtime dependency: `threadpoolctl`.
* The `boundary_forcing` and `initial_conditions` example notebooks gained a quick-start section for the multi-source API, and `end_to_end`, `using_dask` and the datasets overview were updated for the split classes.
