...
* `Grid` topography is now regridded with the same NaN-aware approach as the forcing classes by default: xESMF bilinear interpolation (masked, plus inverse-distance-weighted destination extrapolation for sources with missing values such as `"EMOD"`), with a nearest-neighbor pre-fill + scipy linear interpolation fallback when xESMF is unavailable. The bathymetry `h` shifts slightly for users on the defaults; pass `regrid_method="scipy"` (plus `prefill="2d_lateral_fill"` for `"EMOD"`) to reproduce the legacy output byte-for-byte. ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
...
* `Grid` gains the same regrid/prefill controls as the forcing classes for its topography step: `regrid_method` (`"auto"` (default; xESMF if installed else scipy), `"xesmf"`, `"scipy"`), `prefill` (with `prefill_kwargs`; `None` default = no source fill — only relevant for topography sources with missing values such as `"EMOD"`, since `"ETOPO5"`/`"SRTM15"` are NaN-free), and `extrap_method` (with `extrap_kwargs`) for the default no-prefill path. The options can also be passed to `Grid.update_topography()`. When set, they are recorded in the saved grid metadata and restored by `Grid(filename=...)`; they round-trip through `to_yaml`/`from_yaml`, and the engine actually used is recorded as the `topography_regrid_engine_used` attribute. ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
...
* The `Grid` topography step also uses the shared lateral-regrid helpers, making `Grid` the last class migrated off the legacy always-on 2D fill + scipy interpolation path. ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
