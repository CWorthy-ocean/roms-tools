...
class Dataset:
    ...
    def select_relevant_fields(self, ds) -> xr.Dataset:
        ...
        if missing_vars:
            raise ValueError(f"Dataset does not contain all required variables. The following variables are missing: {missing_vars}")
    ...
    def check_if_global(self, ds) -> bool:
        ...
        """
        dlon_mean = ds[self.dim_names["longitude"]].diff(dim=self.dim_names["longitude"]).mean()
        dlon = (ds[self.dim_names["longitude"]][0] - ds[self.dim_names["longitude"]][-1]) % 360.0
    ...
    def concatenate_longitudes(self, ds):
        ...
        lon_plus360 = lon + 360
        lon_concatenated = xr.concat([lon_minus360, lon, lon_plus360], dim=self.dim_names["longitude"])
        ...
        for var in self.var_names:
            if self.dim_names["longitude"] in ds[var].dims:
                field = ds[var]
                field_concatenated = xr.concat([field, field, field], dim=self.dim_names["longitude"]).chunk({self.dim_names["longitude"]: -1})
