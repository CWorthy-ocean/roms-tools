...
@pytest.mark.parametrize("mock_data_fixture, expected_time_values", [
...
def test_multiple_matching_times(mock_dataset_with_multiple_times_per_day):
    ...
    try:
        # Instantiate Dataset object using the temporary file
        with pytest.raises(ValueError, match="There must be exactly one time matching the start_time. Found 2 matching times."):
            dataset = Dataset(filename=filepath, start_time=start_time)
    finally:
        os.remove(filepath)
...
def test_reverse_latitude_choose_subdomain_negative_depth(mock_dataset):
    ...
    try:
        ...
        dataset.choose_subdomain(latitude_range=(-10, 10), longitude_range=(-10, 10), margin=1, straddle=True)
        ...
        dataset = Dataset(filename=filepath, start_time=start_time)
        dataset.choose_subdomain(latitude_range=(-10, 10), longitude_range=(10, 20), margin=1, straddle=False)
