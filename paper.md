...
7. **BGC surface forcing**: Partial pressure of carbon dioxide, as well as iron, dust, and nitrogen deposition from CESM output [@yeager_2022] or hybrid observational-model sources.
...
While the source datasets listed above (GLORYS, ERA5, SRTM15, TPXO, etc.) are the ones currently supported, the package’s modular design  makes it straightforward to add new data sources or custom routines in the future.
To generate the model inputs listed above, ROMS-Tools automates several intermediate processing steps, including:

* **Bathymetry processing**: The bathymetry is smoothed in two stages, first across the entire model domain and then along the shelf, to ensure local steepness ratios are not exceeded and to reduce pressure-gradient errors. A minimum depth is enforced to prevent water levels from becoming negative during large tidal excursions.
...
Users can quickly design and visualize regional grids and inspect all input fields with built-in plotting utilities. An example of surface initial conditions generated for a California Current System simulation at 5 km horizontal resolution is shown in \autoref{fig:example}.
...
By streamlining input generation and analysis, `ROMS-Tools` reduces technical overhead, lowers the barrier to entry, and enables scientists to focus on research rather than data preparation. The primary users of the package include ocean modelers developing new domains and researchers in the CDR community, who use `ROMS-Tools` to test climate intervention scenarios.
