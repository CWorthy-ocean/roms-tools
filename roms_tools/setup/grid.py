...
class Grid:
    ...
    def plot(self, bathymetry: bool = False) -> None:
        ...
        trans = ccrs.NearsidePerspective(central_longitude=np.mean(lon_deg), central_latitude=np.mean(lat_deg))
...
def _make_grid_ds(
    ...
    ds =  _create_grid_ds(nx, ny, lon4, lat4, pm, pn, ang, size_x, size_y, rot, center_lon, center_lat, lone, late)
...
def _make_initial_lon_lat_ds(domain_length, domain_width, nl, nw):
    ...
    longitude_array_1d_in_degrees = domain_length_in_degrees_longitude * np.arange(-0.5, nl+1.5, 1) / nl - domain_length_in_degrees_longitude /2
    ...
    for it in range(1, 101):
        ...
        latitude_array_1d_in_degrees_cen = 0.5 * (latitude_array_1d_in_degrees[int(np.round(nw/2) + 1)] - latitude_array_1d_in_degrees[int(np.round(nw/2) - 1)])
        ...
        mul = latitude_array_1d_in_degrees_cen/longitude_array_1d_in_degrees_cen * domain_length_in_degrees_longitude / domain_width_in_degrees_latitude  * nw/nl
        latitude_array_1d_in_degrees = latitude_array_1d_in_degrees/mul

    # TODO what does the 'e' suffix mean?
    lon1de = domain_length_in_degrees_longitude * np.arange(-1, nl+2, 1)/nl - domain_length_in_degrees_longitude/2
    ye = (y2 - y1) * np.arange(-1, nw+2)/nw + y1
    #lat1de = 2 * np.arctan(np.exp(ye)) - np.pi/2
    lat1de = np.arctan(np.sinh(ye))
    lat1de = lat1de / mul

    lon1, lat1 = np.meshgrid(longitude_array_1d_in_degrees, latitude_array_1d_in_degrees)
...
def _create_grid_ds(nx, ny, lon, lat, pm, pn, angle, size_x, size_y, rot, center_lon, center_lat, lone, late):
    ...
    #ds['spherical'] = ...
    ds['angle'] = xr.Variable(data=angle, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "Angle between xi axis and east", 'units': "radians"})
    
    #ds['h'] = ...
    # TODO hraw comes from topography
    #ds['hraw'] = xr.Variable(data=hraw, dims=['eta_rho', 'xi_rho'])
    
    ds['f0'] = xr.Variable(data=f0, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "Coriolis parameter at rho-points", 'units': "second-1"})
    ds['pn'] = xr.Variable(data=pn, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "Curvilinear coordinate metric in xi-direction", 'units': "meter-1"})
    ds['pn'] = xr.Variable(data=pm, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "Curvilinear coordinate metric in eta-direction", 'units': "meter-1"})
    
    ds['lon_rho'] = xr.Variable(data=lon * 180 / np.pi, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "longitude of rho-points", 'units': "degrees East"})
    ds['lat_rho'] = xr.Variable(data=lat * 180 / np.pi, dims=['eta_rho', 'xi_rho'], attrs={'long_name': "latitude of rho-points", 'units': "degrees North"})
    ...
    ds['tra_lon'] = xr.Variable(data=[center_lon], dims=['one'], attrs={'long_name': "Longitudinal translation of base grid", 'units': "degrees East"})
    ds['tra_lat'] = xr.Variable(data=[center_lat], dims=['one'], attrs={'long_name': "Latitudinal translation of base grid", 'units': "degrees North"})
    ds['rotate'] = xr.Variable(data=[rot], dims=['one'], attrs={'long_name': "Rotation of base grid", 'units': "degrees"})
