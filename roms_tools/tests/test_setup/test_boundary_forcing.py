...
def test_bgc_bc_with_physics_forcing(use_dask):
    ...
    # Verify the density code path actually changed the output: produce a
    # depth-based comparison and assert at least one BGC variable differs.
    ...
    for direction in ["south", "east", "north", "west"]:
        ...
        if any_diff:
            break
    assert any_diff, "Density interpolation produced identical output to depth-based"
