...
* `SurfaceForcing` with `correct_radiation=True` now scales to long (multi-year) records: the ERA5 radiation-correction climatology is interpolated onto the forcing time axis in chunks, so the correction no longer materializes the full `(time, y, x)` field at once. This also keeps per-timestep access (e.g. validation) cheap. ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
* New "Downloading ERA5 data" section in the datasets documentation shows how to pre-save ERA5 surface forcing (one file per day, NetCDF or Zarr) from the analysis-ready ARCO cloud store for offline/local use. ([#XXX](https://github.com/CWorthy-ocean/roms-tools/pull/XXX))
