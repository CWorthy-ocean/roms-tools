...
@pytest.fixture()
def parent_grid():
    return Grid(nx=5, ny=7, center_lon=-23, center_lat=61, rot=20, size_x=1800, size_y=2400)

@pytest.fixture()
def child_grid():
    return Grid(nx=10, ny=10, center_lon=-23, center_lat=61, rot=-20, size_x=500, size_y=500)

@pytest.fixture()
def parent_grid_that_straddles():
    return Grid(nx=5, ny=7, center_lon=10, center_lat=61, rot=20, size_x=1800, size_y=2400)

@pytest.fixture()
def child_grid_that_straddles():
    return Grid(nx=10, ny=10, center_lon=10, center_lat=61, rot=-20, size_x=500, size_y=500)
...
class TestMapChildBoundaries:
    ...
    def test_correct_indices_of_same_grid(self, grid, caplog, request):
        ...
        for direction in ["south", "east", "north", "west"]:
            ...
            assert "Some boundary points of the child grid are very close to the boundary of the parent grid." in caplog.text
            ...
            if direction == "south":
                ...
                expected_i_xi = (grid.ds.xi_rho[-1] - 0.5) * xr.ones_like(grid.ds.eta_rho)
            elif direction == "north":
                expected_i_eta = (grid.ds.eta_rho[-1] - 0.5) * xr.ones_like(grid.ds.xi_rho)
                expected_i_xi = np.arange(-0.5, grid.ds.xi_rho[-1] + 0.5)
            elif direction == "west":
                expected_i_eta = np.arange(-0.5, grid.ds.eta_rho[-1] + 0.5)
    ...
    def test_indices_are_within_range_of_parent_grid(self, parent_grid_fixture, child_grid_fixture, request):
...
class TestNesting:
    ...
    def test_error_if_child_grid_beyond_parent_grid(self, parent_grid_fixture, child_grid_fixture, request):
        parent_grid = request.getfixturevalue(parent_grid_fixture)
        child_grid = request.getfixturevalue(child_grid_fixture)
        
        with pytest.raises(
            ValueError, match="Some points are outside the grid."
        ):
