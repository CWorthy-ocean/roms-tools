...
* BGC sourcing for `BoundaryForcing` (`type="bgc"`) and `InitialConditions` (`bgc_source=...`) now takes a **single source dict** rather than a `BGCSource` object or a list of them. Use a dataset dict (`{"name": "GLODAP", "path": ...}`) or a constants dict (`{"name": "constants", "constants": {"Fe": 3.0e-3}}`); `climatology` lives in the dict and the source-fill is controlled by the class-level `prefill`/`prefill_kwargs` arguments. To combine several sources, build one forcing object (and output file) per source. The `BGCSource` class has been removed.
* `BoundaryForcing`/`InitialConditions` no longer derive missing MARBL tracers (CHL→phytoplankton expansion, Fe→Lig, alternative-CO2 copies, constant defaults) at construction time. A `type="bgc"` object now contains only the variables its source provides. Derivation and gap-filling are performed by the new `BGCMarbl` model class via `BGCMarbl().process_bgc_fields(forcings, filepath=None)`, which evaluates completeness across the *union* of the supplied forcing objects and writes derived/filled tracers into the object holding the relevant key field (`CHL`/`Fe`). See [#new](https://github.com/CWorthy-ocean/roms-tools/pulls).
